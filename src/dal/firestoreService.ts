...
export const firestoreService = {
  ...
  async updateBook(bookId: string, data: Partial<Book>) {
    ...
    if (normalizedData.programaEducativo) {
      normalizedData.programaEducativo = normalizedData.programaEducativo
        .trim()
        .toUpperCase();
    }
    if (bookId) {
  ...
  async getCommunalRequests() {
    const q = query(
      collection(db, "communalDonationRequests"),
      where("status", "!=", "archived"),
    );
    const snap = await getDocs(q);
    return snap.docs.map(
      (doc) => ({ id: doc.id, ...doc.data() }) as CommunalDonationRequest,
    );
  },

  async addCommunalRequest(request: CommunalDonationRequest) {
    await addDoc(collection(db, "communalDonationRequests"), {
      ...request,
      status: "pending",
    });
  },

  async updateCommunalRequest(
    requestId: string,
    data: Partial<CommunalDonationRequest>,
  ) {
  ...
  subscribeToBooks(callback: (books: Book[]) => void) {
    ...
    return onSnapshot(
      ...
      (snap) => {
        callback(
          snap.docs.map((doc) => ({ id: doc.id, ...doc.data() }) as Book),
        );
      },
      (error) => {
  ...
  subscribeToBookingsByMatricula(
    matricula: string,
  ...
  subscribeToCommunalRequests(
    callback: (requests: CommunalDonationRequest[]) => void,
  ) {
    const q = query(
      collection(db, "communalDonationRequests"),
      where("status", "!=", "archived"),
  ...
  async addCommunalBooking(booking: Omit<CommunalBooking, "id">) {
    return await addDoc(collection(db, "communalBookings"), {
      ...booking,
      status: "active",
    });
  },
  ...
  async getPublicCommunalBookings() {
    const q = query(
      collection(db, "communalBookings"),
      where("type", "==", "public"),
      where("status", "==", "active"),
    );
    const snap = await getDocs(q);
    return snap.docs.map(
      (doc) => ({ id: doc.id, ...doc.data() }) as CommunalBooking,
    );
  },
